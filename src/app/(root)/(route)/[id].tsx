      ...
      const fetchData = async () => {
        try {
          const [fetchedCourses, progress, allRoutes] = await Promise.all([
            ...
            getCourseProgressByRoute(String(id), USER_ID),
            getRoutes(),
          ]);
          ...
          const route = allRoutes.find(
            (r) => r.routeId === parseInt(String(id))
          );
    ...
    <>
      ...
      >
        <View className="mb-6">
          ...
          <Text className="text-base font-semibold text-textPrimary-800 mt-1">
        ...
        {courses.map((course, index) => {
          ...
          const isPreviousCompleted =
            index === 0 ||
            completedCourses.includes(courses[index - 1]?.courseId);
